...
from training.msg import EmptyMessage, FormationRequest, FormationResponse

import csv
import os

class FormationCSVService(BusinessService):
        ...
        with open(os.path.join(self.path, csv_file), "r") as file:
            ...
            for row in reader:
                self.send_request_sync(
                        "Python.FormationBP", 
                        FormationRequest(id=row[0], nom=row[1], salle=row[2])
                    )
